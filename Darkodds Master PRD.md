...
> **PRD version:** 1.1 (post Phase-2 senior engineer review)
...
ERC-7984 wrapper over a test USDC ERC-20.

```solidity
// PSEUDOCODE — agent must consult OpenZeppelin reference + iExec Nox library
contract ConfidentialUSDC is ERC7984 {
  IERC20 public immutable underlying;

  function wrap(uint256 amount) external returns (bytes32 handle);
  function unwrap(bytes32 handle, uint256 amount) external;
...
- `wrap`: pulls `amount` of underlying USDC from user, encrypts via Nox, mints handle, grants ACL to user
- `unwrap`: TEE attests to handle balance, burns handle, transfers underlying to user
- `confidentialTransfer`: standard ERC-7984 confidential transfer with sender/receiver ACL

Use the OpenZeppelin Confidential Contracts library as the base. Do not reinvent. Wrap their `ERC7984` and add the `wrap`/`unwrap` over an underlying ERC-20.
...
The TEE handlers are implemented per the Nox SDK conventions. Agent must consult the official Nox docs (§15) for the current handler deployment pattern — this PRD specifies behavior, not implementation file format.
...
Before any contract or frontend work, the agent runs a health check script:

1. `npm install @iexec-nox/handle` (or current package per docs)
2. Connect to Arbitrum Sepolia
3. Call `encryptInput(42)` — confirm handle returned
4. Call `decrypt(handle)` — confirm 42 returned
5. Confirm Handle Gateway is reachable
6. Document result in `feedback.md` + `DRIFT_LOG.md`

If health check fails: stop. Do not proceed. Investigate. Reach out via iExec Discord. The fallback (recorded TEE responses) is for the demo, NOT for development — develop against real infra.

**Deliverable:** `tools/healthcheck.ts` script that prints "GREEN" and a summary, plus a logged entry in `feedback.md` describing the experience.
...
- **iExec Nox SDK:** `@iexec-nox/handle` latest
- **Recharts:** latest
