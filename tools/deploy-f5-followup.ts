...
//
// F5-followup — Deploy patched MarketImplementation v5 (empty-winning-side
// auto-Invalid in freezePool) and point the registry at it via the Safe
// multisig.
//
// What it does:
//   1. Compiles contracts via forge.
//   2. Deploys MarketImplementation v5 (F5-followup edge case fix).
//   3. Reads the production MarketRegistry address from deployments.
//   4. Builds a Safe tx setting the registry's marketImplementation to v5.
//   5. Signs with PK1 + PK2 (2-of-3) and executes.
//   6. Verifies registry.marketImplementation() == v5 on-chain.
//   7. Writes the new address into deployments json under
//      `contracts.MarketImplementation_v5`.
//   8. Verifies MarketImpl v5 on Arbiscan via Etherscan V2 API.
//
...
function verifyOnArbiscan(addr: string, contractPath: string, contractName: string) {
  ...
  if (!apiKey) {
    console.log(`[deploy-f5fu] no ARBISCAN_API_KEY, skipping verification`);
    return;
  }
  const cmd =
  ...
  try {
    ...
    console.warn(`[deploy-f5fu] verify warning:`, err instanceof Error ? err.message : err);
  }
}
...
async function main() {
  ...
  console.log(`[deploy-f5fu] Deployer: ${account.address}`);
  const bal = await pub.getBalance({address: account.address});
  console.log(`[deploy-f5fu] Balance:  ${formatEther(bal)} ETH`);
  if (bal < 5_000_000_000_000_000n) throw new Error("balance < 0.005 ETH; top up");

  console.log(`[deploy-f5fu] forge build...`);
  ...
  console.log(`[deploy-f5fu] Safe:     ${safeAddress}`);
  console.log(`[deploy-f5fu] Registry: ${registryAddress}`);

  // ----- 1. Deploy MarketImpl v5 (idempotent) -----
  let marketImplV5: Hex;
  if (dep.contracts.MarketImplementation_v5) {
    marketImplV5 = dep.contracts.MarketImplementation_v5;
    const code = await pub.getCode({address: marketImplV5});
    if (code && code.length > 2) {
      console.log(`[deploy-f5fu] reusing existing MarketImplementation_v5: ${marketImplV5}`);
    } else {
      throw new Error(`stale MarketImplementation_v5 entry; remove from deployments json`);
    }
  } else {
    const art = JSON.parse(readFileSync(`${process.cwd()}/contracts/out/Market.sol/Market.json`, "utf8")) as {
      bytecode: {object: Hex};
    };
    console.log(`[deploy-f5fu] deploying MarketImplementation v5...`);
    ...
    if (deployRc.status !== "success" || !deployRc.contractAddress) {
      throw new Error(`MarketImpl v5 deploy failed: ${deployHash}`);
    }
    marketImplV5 = deployRc.contractAddress as Hex;
    console.log(`[deploy-f5fu] MarketImplementation_v5: ${marketImplV5}  (tx ${deployHash})`);
  }
  ...
  console.log(`[deploy-f5fu] preparing Safe tx: registry.setMarketImplementation(${marketImplV5})...`);
  ...
  console.log(`[deploy-f5fu] Safe tx executed: ${txHash}`);
  await pub.waitForTransactionReceipt({hash: txHash});

  // ----- 3. Verify registry now points at v5 -----
  const onchainImpl = (await pub.readContract({
    address: registryAddr,
    abi: REGISTRY_ABI,
    functionName: "marketImplementation",
  })) as `0x${string}`;
  if (onchainImpl.toLowerCase() !== marketImplV5.toLowerCase()) {
    throw new Error(`registry impl mismatch: got ${onchainImpl}, expected ${marketImplV5}`);
  }
  console.log(`[deploy-f5fu] registry.marketImplementation() == v5 ✓`);

  // ----- 4. Update deployments json -----
  const oldImpl = dep.contracts.MarketImplementation;
  dep.notes = dep.notes ?? {};
  dep.notes["f5fu_legacy_MarketImplementation_v4"] = (oldImpl ?? "") as string;
  dep.notes["f5fu_marketimpl_swap_tx"] = txHash;
  dep.contracts.MarketImplementation = marketImplV5;
  dep.contracts.MarketImplementation_v5 = marketImplV5;
  dep.arbiscan = dep.arbiscan ?? {};
  dep.arbiscan.MarketImplementation_v5 = `${ARBISCAN_BASE}/address/${marketImplV5}`;
  writeFileSync(
    `${process.cwd()}/contracts/deployments/arb-sepolia.json`,
    JSON.stringify(dep, null, 2) + "\n",
  );
  console.log(`[deploy-f5fu] deployments json updated`);

  // ----- 5. Verify on Arbiscan -----
  console.log(`[deploy-f5fu] verifying MarketImpl v5 on Arbiscan...`);
  verifyOnArbiscan(marketImplV5, "src/Market.sol", "Market");

  console.log(`\n[deploy-f5fu] DONE in ${((Date.now() - t0) / 1000).toFixed(1)}s`);
  console.log(`[deploy-f5fu] MarketImpl v5: ${marketImplV5}`);
  console.log(`[deploy-f5fu] ${ARBISCAN_BASE}/address/${marketImplV5}`);
}

main().catch((e) => {
  console.error("[deploy-f5fu] FAILED:", e);
  process.exit(1);
});
