/**
 * Phase F4.5 smoke test against real Arb Sepolia + real Nox + the live
 * 2-of-3 Safe multisig.
 *
 * Mirrors smoke-f4's two lifecycles but routes every onlyOwner op
 * (createMarket, setAdapter) through `Safe.execTransaction` co-signed by
 * the deployer + signer #2. Proves both that the multisig governance
 * works for production setup AND that the patched MarketImpl v3 is what
 * gets cloned for new markets.
 *
 *   A. YES happy path — fresh PreResolvedOracle market with hardcoded YES.
 *        SAFE.createMarket → CONFIGURE → SAFE.setAdapter →
 *        wrap → setOperator → placeBet(YES) → publishBatch → resolveOracle →
 *        freezePool → claimWinnings
 *
 *   B. INVALID refund path — fresh PreResolvedOracle market with INVALID outcome.
 *        SAFE.createMarket → CONFIGURE → SAFE.setAdapter →
 *        wrap → setOperator → placeBet(YES) → resolveOracle → refundIfInvalid
 *
 * The fresh PreResolvedOracle is deployed by the smoke (deployer-owned)
 * so configure() doesn't need a Safe co-sig.
 */
...
async function main(): Promise<void> {
  ...
  console.log(`[smoke-f45] Deployer: ${account.address}`);
  console.log(
    `[smoke-f45] Safe:     ${deployment.safe.address} (${deployment.safe.threshold}-of-${deployment.safe.signers.length})`,
  );
  console.log(`[smoke-f45] cUSDC v2: ${deployment.contracts.ConfidentialUSDC}`);
  console.log(`[smoke-f45] Registry: ${deployment.contracts.MarketRegistry}`);
  console.log(
    `[smoke-f45] MktImplV3:${deployment.contracts.MarketImplementation_v3 ?? deployment.contracts.MarketRegistry}`,
  );
  console.log(`[smoke-f45] ResOracle:${deployment.contracts.ResolutionOracle}`);
  console.log(`[smoke-f45] PreOracle:${deployment.contracts.PreResolvedOracle}\n`);

  const publicClient = createPublicClient({chain: arbitrumSepolia, transport: http(rpcUrl)});
  const walletClient = createWalletClient({account, chain: arbitrumSepolia, transport: http(rpcUrl)});
  const handleClient = await createViemHandleClient(walletClient);

  // Pre-init Safe SDKs for both signers; reused for every owner-side op.
  ...
  console.log(`[smoke-f45] fresh PreOracle: ${preOracleAddr}`);

  // Replenish tUSDC + wrap once at the top. TestUSDC.mint is onlyOwner
  // (Safe-owned post-F4.5), so the operator multisig has to authorize.
  await safeWrite(
    "safe-mint-tusdc",
  ...
  // ============================================================
  console.log(`\n[smoke-f45] === Lifecycle A: PreResolved YES ===`);
  ...
  const createDataA = encodeFunctionData({
    abi: REGISTRY_ABI,
    functionName: "createMarket",
    args: ["smoke-f45 YES", "PreResolvedOracle hardcoded YES", 2, expiryA, 200n],
  });
  await safeWrite("safe-create-market-A", deployment.contracts.MarketRegistry, createDataA);
  ...
  console.log(`[smoke-f45] Market[${marketIdA}] (A) at ${marketAAddr}`);
  ...
  await timed("wait-batch-A", async () => {
    console.log(`[smoke-f45] sleeping ${BATCH_WAIT_S}s for batch interval...`);
  ...
  await timed("wait-expiry-A", async () => {
    ...
    if (expiry > now) {
      ...
      console.log(`[smoke-f45] sleeping ${wait}s for market expiry...`);
  ...
  console.log(`[smoke-f45] state after resolveOracle: ${stateAfterResolveA} (expect 3 = Resolving)`);
  ...
  const freezeProofs = await timed("freeze-pool-A", async () => {
    ...
    console.log(`[smoke-f45] yes pool plaintext: ${yesPub.value}`);
    console.log(`[smoke-f45] no  pool plaintext: ${noPub.value}`);
  ...
  await timed("wait-claim-window-A", async () => {
    ...
    if (opensAt > now) {
      ...
      console.log(`[smoke-f45] sleeping ${wait}s for claim window...`);
      await new Promise((res) => setTimeout(res, wait * 1000));
    }
  });
  ...
  console.log(`[smoke-f45] Market[${marketIdA}].hasClaimed = true ✓`);

  // ============================================================
  // Lifecycle B — INVALID refund path on a fresh market
  // ============================================================
  console.log(`\n[smoke-f45] === Lifecycle B: PreResolved INVALID → refund ===`);
  ...
  const createDataB = encodeFunctionData({
    abi: REGISTRY_ABI,
    functionName: "createMarket",
    args: ["smoke-f45 INVALID", "PreResolvedOracle hardcoded INVALID", 2, expiryB, 200n],
  });
  await safeWrite("safe-create-market-B", deployment.contracts.MarketRegistry, createDataB);
  ...
  console.log(`[smoke-f45] Market[${marketIdB}] (B) at ${marketBAddr}`);
  ...
  await timed("wait-expiry-B", async () => {
    ...
    if (expiry > now) {
      const wait = Number(expiry - now) + 5;
      console.log(`[smoke-f45] sleeping ${wait}s for market B expiry...`);
      await new Promise((res) => setTimeout(res, wait * 1000));
    }
  });
  ...
  console.log(`[smoke-f45] state after INVALID resolution: ${stateB} (Invalid) ✓`);
  ...
  console.log(`[smoke-f45] yesBet cleared after refund ✓`);

  const totalMs = Math.round(performance.now() - overallStart);
  printSummary(totalMs);
  console.log(
    "GREEN — F4.5 lifecycles A (claim) and B (refund) validated against real Arb Sepolia + Nox + 2-of-3 Safe multisig",
  );
...
main().catch((err) => {
  ...
  printSummary(totalMs);
  console.error(`\n[smoke-f45] FATAL: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}`);
